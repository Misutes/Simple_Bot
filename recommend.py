...
def recommend(event, class_, message, sm, ssm, database, media_1, media_2, media_3):
    ...
    def random_post(text, posts):
        sm(message=text)
        ssm('wall', class_.get_data('group_id'), random.choice(posts))
    ...
    for word in message:
        ...
        if word in quit_:
            ...
            sm(class_.get_data('novelty_bookshelf'), kb.novelty_bookshelf)

        elif word in fantastic:
            random_post(class_.get_data('fantastic'), class_.get_data('fantastic_literature'))

        elif word in japan:
            random_post(class_.get_data('japan'), class_.get_data('japan_literature'))

        elif word in detective:
            random_post(class_.get_data('detective'), class_.get_data('detective_literature'))

        elif word in adult:
            ssm(media_3)
            sm(class_.get_data('adult_bookshelf'), kb.adult_bookshelf)

        elif word in recommendation:
            ssm(media_1)
            sm(class_.get_data('advice_description'), kb.advice_book)
    ...
    if miss_word > len(message) - 1:
        sm(message=class_.get_data('bookshelf_exit'))
